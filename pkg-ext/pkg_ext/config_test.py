...
def test_merge_config_and_write(tmp_path):
    ...
    assert "stability: ga" in content
    assert "docstring: Date utils" in content
    assert "dependencies:" in content
    assert "# generated by pkg-ext from pyproject.toml" in content


def test_deprecated_group_requires_reason():
    with pytest.raises(ValueError, match="deprecation_reason required"):
        GroupConfig(stability=Stability.deprecated)


def test_deprecated_group_with_reason():
    cfg = GroupConfig(stability=Stability.deprecated, deprecation_reason="Use v2")
    assert cfg.deprecation_reason == "Use v2"


def test_deprecation_reason_propagated(tmp_path):
    config = ProjectConfig(
        groups={
            "old": GroupConfig(
                stability=Stability.deprecated, deprecation_reason="Use new_group"
            )
        }
    )
    output_path = tmp_path / ".groups.yaml"
    ...
    assert groups.name_to_group["old"].deprecation_reason == "Use new_group"
    content = output_path.read_text()
    assert "deprecation_reason: Use new_group" in content
