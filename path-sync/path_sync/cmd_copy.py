...
class SyncResult:
    ...
    @property
    def total(self) -> int:
        return self.content_changes + self.tools_changes


@contextmanager
def capture_sync_log(dest_name: str):
    """Context manager to capture sync logs to a temp file."""
...
class CopyOptions:
    dry_run: bool = False
    force_ignore_sha: bool = False
    force_overwrite: bool = False
    scaffold_regen: bool = False
...
def copy(
    ...
    force_ignore_sha: bool = typer.Option(
        False, "--force-ignore-sha-match", help="Ignore src_sha match"
    ),
    ...
    scaffold_regen: bool = typer.Option(False, "--scaffold-regen"),
    validate_first: bool = typer.Option(False, "--validate-first"),
    ...
    config_path = resolve_config_path(src_root, name, SrcConfig)
    ...
    opts = CopyOptions(
        dry_run=dry_run,
        force_ignore_sha=force_ignore_sha,
        force_overwrite=force_overwrite,
        scaffold_regen=scaffold_regen,
...
def _sync_destination(
    ...
    dest_config_path = resolve_config_path(dest_root, config.name, DestConfig)

    dest_config = _load_dest_config(dest_config_path)
    if dest_config and dest_config.src_sha == current_sha and not opts.force_ignore_sha:
        logger.info(f"{dest.name}: Already at {current_sha[:8]}, skipping")
        return 0
    ...
    if opts.validate_first and dest_config:
        unauthorized = validate_dest_config(dest_root, dest_config)
        if unauthorized:
            files_list = "\n  ".join(unauthorized)
            raise ValueError(f"{dest.name}: unauthorized changes:\n  {files_list}")
    ...
    if result.content_changes == 0 and not opts.force_tools_pr:
        logger.info(f"{dest.name}: Tools-only changes, skipping PR")
        return 0

    _write_dest_config(
        dest_config_path, config, dest, current_sha, src_repo_url, opts.dry_run
    )
...
def _load_dest_config(path: Path) -> DestConfig | None:
    if not path.exists():
        return None
    return load_yaml_model(path, DestConfig)
...
def _sync_paths(
    ...
    tools_changes = _sync_tools_update(config, dest, dest_root, opts)
    return SyncResult(content_changes=content_changes, tools_changes=tools_changes)
...
def _sync_tools_update(
    ...
    if dest.tools_update.path_sync_wheel:
        _sync_wheel(dest_root, opts)  # Silent: always copy, never count
...
def _sync_wheel(dest_root: Path, opts: CopyOptions) -> None:
    """Sync wheel to destination. Always copies, never counts as a change."""
...
def _write_dest_config(
    path: Path, config: SrcConfig, dest: Destination, sha: str, url: str, dry_run: bool
) -> None:
    new_config = DestConfig(
        src_name=config.name,
        dest_name=dest.name,
        default_branch=dest.default_branch,
        header_config=config.header_config,
        always_paths=config.always_paths,
        scaffold_paths=config.scaffold_paths,
        src_sha=sha,
        src_repo_url=url,
    )
    if not dry_run:
        ensure_parents_write_text(path, dump_yaml_model(new_config))
...
def _sync_scaffold_path(
    ...
    src_path = src_root / mapping.src_path
    dest_path = dest_root / mapping.resolved_dest_path()

    if dest_path.exists() and not regen:
        return 0

    if not src_path.exists():
        logger.warning(f"Scaffold source not found: {mapping.src_path}")
        return 0
    ...
    if dry_run:
        logger.info(f"[DRY RUN] Would scaffold: {dest_path}")
        return 1

    ensure_parents_write_text(dest_path, content)
    logger.info(f"Scaffolded: {dest_path}")
    return 1
